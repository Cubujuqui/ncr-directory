...
function Avatar({ dark }: { dark: string }) {
  return (
    <div style={{ width: '70px', height: '70px', borderRadius: '50%', border: `1.5px dashed ${dark}66`, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundImage: `repeating-linear-gradient(135deg, ${dark}14 0 6px, ${dark}05 6px 12px)`, flexShrink: 0 }}>
      <span style={{ fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace", fontSize: '9px', color: `${dark}99`, textAlign: 'center' }}>foto</span>
    </div>
  );
}
...
function PremiumCard({ perfil, paleta }: { perfil: PerfilCompleto; paleta: typeof PALETAS[number] }) {
  ...
  return (
    <div style={{ background: paleta.light, borderRadius: '18px', padding: '22px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', boxShadow: '0 8px 20px rgba(0,0,0,0.08)', height: '100%', boxSizing: 'border-box' }}>
      <span style={{ display: 'inline-block', background: paleta.primary, color: '#ffffff', fontSize: '11px', fontWeight: 800, padding: '3px 10px', borderRadius: '999px', marginBottom: '14px', letterSpacing: '0.3px' }}>
        PERFIL PREMIUM
      </span>
      <Avatar dark={paleta.dark} />
      <p style={{ margin: '14px 0 0', fontWeight: 800, fontSize: '16px', color: paleta.dark }}>{nombreCompleto}</p>
